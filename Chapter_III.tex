...
\end{problem}
\begin{pf}
The universal property is as the same stated in I.5.12, but by replacing every set with $R$-modules and every morphism with $R-\mathsf{Mod}$ homomorphisms. We now show that the fibered coproduct is almost the same in \textsf{Set}: define an equivalence relation
\[
R = \{(\mu(x), \nu(x)) \in M \oplus N : x \in A\}
\]
on $M \oplus N$, and let $M \oplus_A N := (M \oplus N)/R$. We show that $R$ is a submodule, so the quotient make sense. For $(m,n) \in R$,
\[
r(m,n) = r(\mu(x),\nu(x)) = (r\mu(x), r\nu(x)) = (\mu(rx), \nu(rx)) \in R	
\]
which shows that the quotient is well-defined. Now define
\[
\varphi((m,n)+R) = f_M(m) + f_N(n)	
\]
It is a simple check that $\varphi$ is a $R$-module homomorphism, and $\varphi$ is well-defined (and is unique) by the universal property of quotient. This makes the following diagram 
\[
\begin{tikzcd}
...
\item $i_N \nu = i_M \mu$:
\[
i_N \nu (x) = (0, \nu(x)) + R = (\mu(x), 0) + R = i_M \mu(x)
\]
\item $f_M = \varphi i_M $ (resp. $f_N = \varphi i_N$):
\[
\varphi	i_M (m) = \varphi((m,0)+R) = f_M(m).
\]
\end{itemize}
\end{pf}
...
\]
so we have the candidate of $f$. Now it remains to decide $m$ in which $i(m) = n - \beta(\varphi(n))$: notice that by exactness, $\im i = \ker \varphi$, so we check that $\varphi(n - \beta(\varphi(n))) = 0$ to gurantee the existence of $m$:
\[
